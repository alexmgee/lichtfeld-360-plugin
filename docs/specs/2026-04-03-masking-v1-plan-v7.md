# PanoSplat Masking v1 — Implementation Plan v7

> **Date:** 2026-04-03
> **Supersedes:** All prior plans (v1 through v6)
> **Spec:** `2026-04-02-masking-layer-v1-design.md` (to be updated in Milestone 2)
> **Review:** Incorporates feedback from all inspection documents through `docs/2026-04-03-masking-v1-plan-v6-inspection.md`
...
- `_synthetic_pass(frame_files, primary_masks, directions, frame_order)` → `dict[str, np.ndarray]` — render synthetic fisheye, track/detect, backproject. Owns the `best_frame_idx` selection (e.g. frame with largest primary mask area) and passes it to the video backend as `initial_frame_idx`.
