...
**Architecture:** Two-tier detection backend (YOLO+SAM v1 default, SAM 3 user-selected upgrade) runs on cubemap-decomposed faces. Masks merge back to ERP space, reframer reprojects to pinhole views, closest-camera Voronoi masks eliminate duplicate COLMAP features. Pipeline order: Extract → Mask → Reframe → COLMAP.
...
**Venv management:** Use `uv add`/`uv remove` for dependencies. NEVER use `uv pip install` — it can corrupt the venv by creating package conflicts.
...
| `core/backends.py` | `MaskingBackend` protocol + YOLO+SAM v1 and SAM 3 implementations |
...
| File | Changes |
|------|---------|
| `core/masker.py` | Replace video-mode SAM 3 with cubemap decomposition + backend dispatch + postprocessing |
...
| `panels/prep360_panel.py` | Re-enable masking UI, target checkbox, install/upgrade UX |

### Already Complete
| File | Status |
|------|--------|
| `lib/python3.dll` | Bundled — confirmed working 2026-04-03 |
| `__init__.py` | DLL directory setup — confirmed working 2026-04-03 |
...
Create the backend protocol and both backend implementations.
    ...
    def detect_and_segment(
        ...
        for prompt_text in targets:
            try:
                ...
                if masks is None:
                    continue
                # Union all masks for this prompt
                for mask in masks:
                    if hasattr(mask, "cpu"):
                        arr = mask.cpu().numpy()
                    else:
...
def get_backend(preference: str | None = None) -> MaskingBackend | None:
    ...
    # Default: return default tier if available
...
git commit -m "feat: add two-tier masking backends (YOLO+SAM v1, SAM 3)"
```
...
Replace the existing video-mode SAM 3 masker with the cubemap decomposition pipeline that uses the backend interface.
...
from .backends import get_backend, get_backend_name, MaskingBackend
...
class Masker:
    ...
    def initialize(self) -> None:
        """Load the user's chosen detection backend."""
    ...
    def process_frames(
        ...
        # Cubemap face → view name mapping (must match presets.py cubemap ring naming)
        # Ring 0 (pitch=0, count=4, start_yaw=0): front=00_00, right=00_01, back=00_02, left=00_03
        # Ring 1 (pitch=-90, count=1): down=01_00
        # Ring 2 (pitch=+90, count=1): up=02_00
        face_to_view = {
...
def active_backend(self) -> str | None:
    """Return the active backend based on what's installed.
    Note: SAM 3 is only active if the user has explicitly installed it."""
...
Add `install_default_tier(on_output=None) -> bool` that runs:
`uv add ultralytics segment-anything torch torchvision` (with CUDA index)
and eagerly downloads SAM v1 ViT-H weights after install.
...
- Pass `is_cubemap=(cfg.preset_name == "cubemap")`, `output_size`, and `backend_preference` to `MaskConfig`
...
- Replace SAM 3-only setup with two-tier UX:
  - Default: "Enable Masking" → one-click install if deps missing
  - Premium: "Upgrade to SAM 3" link → opens setup wizard
- Add "person" target checkbox — bind to `_mask_targets`
- Show active backend indicator ("Using YOLO+SAM v1" or "Using SAM 3")
- Wire `install_default_tier` to install button

- [ ] **Step 3: Update prep360_panel.rml**

Add masking section with:
- Enable masking toggle
- Person target checkbox
...
## Task 9: Integration Test — End-to-End Masking
