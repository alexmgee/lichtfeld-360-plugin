...
def test_dense_preset_mixed_angle_views_point_where_their_angles_say():
    """Dense preset views should honour both yaw and pitch simultaneously."""
    for yaw, pitch, _fov, _name, _flip in VIEW_PRESETS["dense"].get_all_views():
        R = create_rotation_matrix(yaw, pitch)
        forward = R @ np.array([0.0, 0.0, 1.0])
        expected = np.array(
            [
